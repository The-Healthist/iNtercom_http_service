...
type InterfaceStaffService interface {
	GetAllStaff(page, pageSize int, search string) ([]model.PropertyStaff, int64, error)
	GetStaffByID(id uint) (*model.PropertyStaff, error)
	CreateStaff(staff *model.PropertyStaff) error
	UpdateStaff(id uint, updates map[string]interface{}) (*model.PropertyStaff, error)
	DeleteStaff(id uint) error
	GetStaffDevices(staffID uint) ([]model.Device, error)
	GetStaffByIDWithDevices(id uint) (*model.PropertyStaff, error)
}
...
func (s *StaffService) GetAllStaff(page, pageSize int, search string) ([]model.PropertyStaff, int64, error) {
	var staff []model.PropertyStaff
	var total int64

	query := s.DB.Model(&model.PropertyStaff{})
...
func (s *StaffService) GetStaffByID(id uint) (*model.PropertyStaff, error) {
	var staff model.PropertyStaff
...
func (s *StaffService) CreateStaff(staff *model.PropertyStaff) error {
	// 验证手机号唯一性
	var count int64
	if err := s.DB.Model(&model.PropertyStaff{}).Where("phone = ?", staff.Phone).Count(&count).Error; err != nil {
		return err
	}
	if count > 0 {
	...
	// 验证用户名唯一性
	if err := s.DB.Model(&model.PropertyStaff{}).Where("username = ?", staff.Username).Count(&count).Error; err != nil {
		return err
	}
	if count > 0 {
...
func (s *StaffService) UpdateStaff(id uint, updates map[string]interface{}) (*model.PropertyStaff, error) {
	...
	if phone, ok := updates["phone"].(string); ok && phone != staff.Phone {
		var count int64
		if err := s.DB.Model(&model.PropertyStaff{}).Where("phone = ? AND id != ?", phone, id).Count(&count).Error; err != nil {
			return nil, err
		}
		if count > 0 {
	...
	if username, ok := updates["username"].(string); ok && username != staff.Username {
		var count int64
		if err := s.DB.Model(&model.PropertyStaff{}).Where("username = ? AND id != ?", username, id).Count(&count).Error; err != nil {
			return nil, err
		}
		if count > 0 {
...
func (s *StaffService) GetStaffDevices(staffID uint) ([]model.Device, error) {
	// 检查物业人员是否存在
...
func (s *StaffService) GetStaffByIDWithDevices(id uint) (*model.PropertyStaff, error) {
	staff, err := s.GetStaffByID(id)
	if err != nil {
		return nil, err
	}

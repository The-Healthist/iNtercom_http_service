...
type InterfaceAdminService interface {
	...
	GetAllAdmins(page, pageSize int, search string) ([]model.Admin, int64, error)
	CreateAdmin(admin *model.Admin) error
	UpdateAdmin(id uint, updates map[string]interface{}) (*model.Admin, error)
...
func (s *AdminService) GetAllAdmins(page, pageSize int, search string) ([]model.Admin, int64, error) {
...
func (s *AdminService) CreateAdmin(admin *model.Admin) error {
	// 验证用户名唯一性
	var count int64
	if err := s.DB.Model(&model.Admin{}).Where("username = ?", admin.Username).Count(&count).Error; err != nil {
		return err
	}
	if count > 0 {
...
func (s *AdminService) UpdateAdmin(id uint, updates map[string]interface{}) (*model.Admin, error) {
	// 首先获取管理员
	...
	if username, ok := updates["username"].(string); ok && username != admin.Username {
		var count int64
		if err := s.DB.Model(&model.Admin{}).Where("username = ? AND id != ?", username, admin.ID).Count(&count).Error; err != nil {
			return nil, err
		}
		if count > 0 {
...
func (s *AdminService) DeleteAdmins(ids []uint) error {
	...
	if err := s.DB.Model(&model.Admin{}).Where("id IN ?", ids).Count(&deleteCount).Error; err != nil {
		return err
	}
	...
	result := s.DB.Delete(&model.Admin{}, ids)
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected == 0 {

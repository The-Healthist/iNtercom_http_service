...
type InterfaceCallRecordService interface {
	GetAllCallRecords(page, pageSize int) ([]model.CallRecord, int64, error)
	GetCallRecordByID(id uint) (*model.CallRecord, error)
	GetCallRecordsByDeviceID(deviceID uint, page, pageSize int) ([]model.CallRecord, int64, error)
	GetCallRecordsByResidentID(residentID uint, page, pageSize int) ([]model.CallRecord, int64, error)
	GetCallStatistics() (*CallStatistics, error)
	SubmitCallFeedback(feedback *CallFeedback) error
	GetCallRecordByCallID(callID string) (*model.CallRecord, error)
}
...
func (s *CallRecordService) GetAllCallRecords(page, pageSize int) ([]model.CallRecord, int64, error) {
	var calls []model.CallRecord
	var total int64

	// 获取总数
	if err := s.DB.Model(&model.CallRecord{}).Count(&total).Error; err != nil {
		return nil, 0, err
	}
...
func (s *CallRecordService) GetCallRecordByID(id uint) (*model.CallRecord, error) {
...
func (s *CallRecordService) GetCallRecordsByDeviceID(deviceID uint, page, pageSize int) ([]model.CallRecord, int64, error) {
	var calls []model.CallRecord
	var total int64
	...
	// 获取总数
	if err := s.DB.Model(&model.CallRecord{}).Where("device_id = ?", deviceID).Count(&total).Error; err != nil {
		return nil, 0, err
	}
...
func (s *CallRecordService) GetCallRecordsByResidentID(residentID uint, page, pageSize int) ([]model.CallRecord, int64, error) {
	var calls []model.CallRecord
	var total int64
	...
	// 获取总数
	if err := s.DB.Model(&model.CallRecord{}).Where("resident_id = ?", residentID).Count(&total).Error; err != nil {
		return nil, 0, err
	}
...
func (s *CallRecordService) GetCallStatistics() (*CallStatistics, error) {
	...
	// 获取总通话数
	if err := s.DB.Model(&model.CallRecord{}).Count(&statistics.TotalCalls).Error; err != nil {
		return nil, err
	}

	// 获取已接通话数
	if err := s.DB.Model(&model.CallRecord{}).Where("call_status = ?", model.CallStatusAnswered).Count(&statistics.AnsweredCalls).Error; err != nil {
		return nil, err
	}

	// 获取未接通话数
	if err := s.DB.Model(&model.CallRecord{}).Where("call_status = ?", model.CallStatusMissed).Count(&statistics.MissedCalls).Error; err != nil {
		return nil, err
	}

	// 获取超时通话数
	if err := s.DB.Model(&model.CallRecord{}).Where("call_status = ?", model.CallStatusTimeout).Count(&statistics.TimeoutCalls).Error; err != nil {
		return nil, err
	}
	...
	if statistics.AnsweredCalls > 0 {
		...
		if err := s.DB.Model(&model.CallRecord{}).
			Where("call_status = ?", model.CallStatusAnswered).
...
func (s *CallRecordService) GetCallRecordByCallID(callID string) (*model.CallRecord, error) {
	var call model.CallRecord

...
func NewCallRecordController(ctx *gin.Context, container *service.ServiceContainer) *CallRecordController {
...
func HandleCallRecordFunc(container *service.ServiceContainer, method string) gin.HandlerFunc {
...
func (c *CallRecordController) GetCallRecords() {
	...
	callRecordService := c.Container.GetService("call_record").(service.InterfaceCallRecordService)

	calls, total, err := callRecordService.GetAllCallRecords(page, pageSize)
	if err != nil {
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "获取通话记录失败: "+err.Error(), nil)
		return
	}
...
func (c *CallRecordController) GetCallRecord() {
	...
	if err != nil {
		errcode.ParamError(c.Ctx, "无效的通话记录ID")
		return
	}
...
func (c *CallRecordController) GetCallStatistics() {
	...
	if err != nil {
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "获取通话统计信息失败: "+err.Error(), nil)
		return
	}

	errcode.Success(c.Ctx, statistics)
}
...
func (c *CallRecordController) GetDeviceCallRecords() {
	...
	if err != nil {
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "获取设备通话记录失败: "+err.Error(), nil)
		return
	}
...
func (c *CallRecordController) GetResidentCallRecords() {
	...
	if err != nil {
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "获取居民通话记录失败: "+err.Error(), nil)
		return
	}
...
func (c *CallRecordController) SubmitCallFeedback() {
	...
	if err := callRecordService.SubmitCallFeedback(feedback); err != nil {
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "提交反馈失败: "+err.Error(), nil)
		return
	}
...
func (c *CallRecordController) GetCallSession() {
	...
	if callID == "" {
		errcode.ParamError(c.Ctx, "缺少必要的call_id参数")
		return
	}
	...
	if err != nil {
		errcode.NotFound(c.Ctx, "未找到通话记录: "+err.Error())
		return
	}

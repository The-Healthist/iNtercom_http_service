...
func NewStaffController(ctx *gin.Context, container *service.ServiceContainer) *StaffController {
...
func (c *StaffController) GetStaffs() {
	...
	staffService := c.Container.GetService("staff").(service.InterfaceStaffService)

	staffs, total, err := staffService.GetAllStaff(page, pageSize, search)
	if err != nil {
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "查询物业员工列表失败: "+err.Error(), nil)
		return
	}
...
func (c *StaffController) GetStaff() {
	...
	if err != nil {
		if err.Error() == "物业员工不存在" {
			errcode.NotFound(c.Ctx, "物业员工不存在")
			return
		}
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "查询物业员工失败: "+err.Error(), nil)
		return
	}
...
func HandleStaffFunc(container *service.ServiceContainer, method string) gin.HandlerFunc {

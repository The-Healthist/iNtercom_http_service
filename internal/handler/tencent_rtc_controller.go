...
func NewTencentRTCController(ctx *gin.Context, container *service.ServiceContainer) *TencentRTCController {
...
func (c *TencentRTCController) GetUserSig() {
	...
	// 获取腾讯云RTC服务
	tencentRTCService := c.Container.GetService("tencent_rtc").(service.InterfaceTencentRTCService)

	// 生成UserSig
	tokenInfo, err := tencentRTCService.GetUserSig(req.UserID)
	if err != nil {
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "生成UserSig失败: "+err.Error(), nil)
		return
	}
...
func (c *TencentRTCController) StartTencentCall() {
	...
	if err != nil {
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "发起通话失败: "+err.Error(), nil)
		return
	}
	...
	if err != nil {
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "生成设备UserSig失败: "+err.Error(), nil)
		return
	}
	...
	if err != nil {
		errcode.FailWithMessage(c.Ctx, errcode.ErrDatabase, "生成居民UserSig失败: "+err.Error(), nil)
		return
	}
...
func HandleTencentRTCFunc(container *service.ServiceContainer, method string) gin.HandlerFunc {

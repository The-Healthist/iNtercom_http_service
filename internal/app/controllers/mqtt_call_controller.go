...
func (c *MQTTCallController) HandleError(status int, message string, err error) {
	...
	case http.StatusBadRequest:
		response.FailWithMessage(c.Ctx, code.ErrBind, errMessage, nil)
	case http.StatusNotFound:
		response.NotFound(c.Ctx, errMessage)
	case http.StatusInternalServerError:
		response.FailWithMessage(c.Ctx, code.ErrDatabase, errMessage, nil)
	default:
		response.FailWithMessage(c.Ctx, code.ErrDatabase, errMessage, nil)
	}
}

...
func (c *ResidentController) GetResidents() {
	...
	if err != nil {
		response.FailWithMessage(c.Ctx, code.ErrDatabase, "获取居民列表失败", nil)
		return
	}
...
func (c *ResidentController) GetResident() {
	id := c.Ctx.Param("id")
	if id == "" {
		response.ParamError(c.Ctx, "居民ID不能为空")
		return
	}
	...
	if err != nil {
		...
		response.FailWithMessage(c.Ctx, code.ErrDatabase, "获取居民信息失败", nil)
		return
	}

	response.Success(c.Ctx, resident)
}
...
func (c *ResidentController) CreateResident() {
	...
	if err := residentService.CreateResident(resident); err != nil {
		if err.Error() == "手机号已被使用" || err.Error() == "户号不存在" {
			response.FailWithMessage(c.Ctx, code.ErrBind, err.Error(), nil)
			return
		}
		response.FailWithMessage(c.Ctx, code.ErrDatabase, "创建居民失败: "+err.Error(), nil)
		return
	}
...
func (c *ResidentController) UpdateResident() {
	...
	if err != nil {
		...
		response.FailWithMessage(c.Ctx, code.ErrDatabase, "更新居民失败: "+err.Error(), nil)
		return
	}
...
func (c *ResidentController) DeleteResident() {
	...
	if err := residentService.DeleteResident(uint(idUint)); err != nil {
		...
		response.FailWithMessage(c.Ctx, code.ErrDatabase, "删除居民失败: "+err.Error(), nil)
		return
	}

...
func (c *RTCController) GetToken() {
	...
	if err != nil {
		response.FailWithMessage(c.Ctx, code.ErrDatabase, "生成令牌失败", nil)
		return
	}
...
func (c *RTCController) StartCall() {
	...
	if err != nil {
		response.FailWithMessage(c.Ctx, code.ErrDatabase, "发起通话失败", nil)
		return
	}
	...
	if err != nil {
		response.FailWithMessage(c.Ctx, code.ErrDatabase, "生成设备令牌失败", nil)
		return
	}

	residentToken, err := rtcService.GetToken(channelID, req.ResidentID)
	if err != nil {
		response.FailWithMessage(c.Ctx, code.ErrDatabase, "生成居民令牌失败", nil)
		return
	}
